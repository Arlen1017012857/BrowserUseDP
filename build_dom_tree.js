 ...
 * This code is sourced from the browser-use project repository.
 * Get clickable elements on the page
 *
 * @param {*} doHighlightElements Is highlighted
 * @param {*} includeAttributes [attr_names...]
 * @param {*} containerSelector Optional CSS selector to filter elements by container
 * @returns { element_str, selector_map }
 */
function get_clickable_elements(doHighlightElements = true, includeAttributes, containerSelector = null) {
    window.clickable_elements = {};
    let page_tree = build_dom_tree(doHighlightElements, containerSelector);
...
function clickable_elements_to_string(element_tree, includeAttributes) {
    ...
    function get_all_text_till_next_clickable_element(element_node) {
        ...
        let text_result = text_parts.join(' ').trim().replace(/\s+/g, ' ');
        if (!text_result && element_node.highlightIndex != null && window.clickable_elements) {
            let domEl = window.clickable_elements[element_node.highlightIndex];
            if (domEl && domEl.innerText) {
                text_result = domEl.innerText.trim();
            }
        }
        return text_result;
    }
    ...
    function process_node(node, depth) {
        if (node.text == null) {
            let initialCount = formatted_text.length;
            for (let i = 0; i < node.children.length; i++) {
                process_node(node.children[i], depth + 1);
            }
            if (node.highlightIndex != null && formatted_text.length === initialCount) {
                let attributes_str = '';
                if (includeAttributes) {
            ...
            if (node.text.trim() !== '') {
                formatted_text.push(`[]:${node.text.trim()}`);
            }
        }
    }
...
function build_dom_tree(doHighlightElements, containerSelector = null) {
    let highlightIndex = 0; // Reset highlight index
    let containerElement = null;

    // If a container selector is provided, find that element
    ...
    function isTopElement(element) {
        ...
        // If we have a container element, only consider elements within it
        if (containerElement && !isElementInContainer(element)) {
            return false;
        }
    ...
    // Helper function to check if element is within the specified container
    function isElementInContainer(element) {
        if (!containerElement) return true; // If no container specified, accept all elements

        // Check if element is within the container
        let current = element;
        while (current && current !== document.documentElement) {
            if (current === containerElement) return true;
            current = current.parentElement;
        }
        return false;
    }
    ...
    function buildDomTree(node, parentIframe = null) {
        ...
        // Check if element is in the container (if specified)
        if (containerElement && node.nodeType === Node.ELEMENT_NODE && !isElementInContainer(node) && node !== containerElement) {
            // Only process the container element itself or elements inside it
            return null;
        }
